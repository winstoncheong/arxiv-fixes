...
\address{Max-Planck-Institut f\"ur Mathematik, Bonn \\
and University of California, Berkeley\\
Dept. of Mathematics, University of California, Berkeley, CA 94720, USA}
...
% make subsection numbering bold.
...
through 1.1 and to the statements of theorems occurring in 1.3.1-1.3.2.
...
map $f:C\to V$ having no non-trivial first order infinitesimal automorphisms,
...
\begin{notation} $\M_{g,k}(V,\beta)$ denotes the moduli stack
...
stable maps to $V$ as a flat proper morphism $\Cal C\to S$
 to a scheme $S$ of finite type over the ground field and a map
$f:\Cal C\to V$ such that its restriction to each geometric fiber of $\Cal C$
...
Functors between orbispaces which are continuous, locally homeomorphisms and
...
\subsection{Properties of moduli spaces of stable maps}
...
ordinary double singular points and a map $f:C\to V$. P.~Pansu claimed in [P]
...
\subsubsection{Algebraicity and properness}



\begin{thm} Let $V$ be a projective scheme of finite type over a field.
Then
...
separatedness by definition.
 ...
 If we have a family of stable maps  $\Cal C/K,\,\,f:\Cal C\to V$ over a
...
\begin{thm} Let $V$ be a smooth proper scheme of finite type over a
...
Stable maps have the following important property.
  Let us consider  a flat proper morphism $\Cal C\to S$
...
$f:\Cal C\to  V$. We claim that the set of points $p$ of $S$ such that the
...
of $C$ are given by
  $$\bold H^1(C,\Cal T'_{C}\to f^*\Cal T_V)\,,$$
...
$$0\to \mathbf H^1(C,\Cal F^{\bullet})\to T\to \bigoplus_
{y\in C^{sing}} T_y^1C\otimes T_y^2C\to \mathbf H^2(C,\Cal F^{\bullet})\,\,\,,$$
...
We are ready now for the proof of the smoothness criterion.
...
\subsection{The structure of an intersection of manifolds}
...
Also, if we have two maps of manifolds
 $Y_i \xrightarrow{f_i} X,\,\,i=1,2$ then the fiber product $Z:=Y_1\times_X Y_2$ carries a
...
tangent spaces
$$T_zY_1\oplus T_zY_2\to T_zX\,,\,\,\,z\in Z\,\,,$$
is Fredholm.
...
\subsubsection{Reduction to finite dimensional manifolds}
...
T_{Y_2})_{|Z}\,\,\,.$$
We can prolong $\Cal E_i$ to  neighborhoods of $Z$ in $Y_i$.
...
\subsubsection{Intersections of manifolds in algebraic geometry}
...
and Hilbert schemes should carry canonical structures of quasi-manifolds.
  The idea of introducing higher structure sheaves on moduli spaces occurred
...
$$\Phi:\M_{g,k}(V,\beta)\to V^k\times\M_{g,k}\,,$$
...
\subsection{Comparison with other definitions}
...
\subsection{Rational curves on $\P^2$}
...
degree of the map $$\phi:\M_{0,k}(\P^2,d)\to (P^2)^k,\qquad
...
\subsection{Rational curves on quintics}
...
The orbispace $\M_{0,0}(V,d[\P^1])$ is a subspace of $\M(\P^4,d)$.
 ...
 $\M_{0,1}(\P^4,d)\to\M_{0,0}(\P^4,d)=\M(\P^4,d)$ of $\phi^*(\O(5))$.
  Here again $\phi(C;x_1;f)=f(x_1)\in \P^4$.
 The sheaf $\Cal E_d$ is actually a vector bundle:
  for any stable map $f:C\to\P^4$ from a curve of arithmetic genus zero
...
%% FOLLOWING LINE CANNOT BE BROKEN BEFORE 80 CHAR
%% FOLLOWING LINE CANNOT BE BROKEN BEFORE 80 CHAR
\frac{2875}{1}&\frac{4876875}{8}&\frac{8564575000}{27}&\frac{15517926796875}{64}\end{matrix}$$
...
$$F(t):=\frac{5}{6}t^3+\sum_{d\ge 1} N_d e^{dt}\,\,.$$
 ...
 $$G(q_1,q_2)=F(q_1/q_2)\,q_2^2\,\,.$$
  The function $G$ generates a Lagrangian cone $\Cal L$ in the symplectic
vector space $\C^4$:
...
$1$-dimensional terms of the Hodge filtration $F^3_z$ forms a Lagrangian cone
in $\C^4$.
...
\subsection{Multiple coverings of rational curves on Calabi-Yau 3-folds}
...
$\M_{0,0}(C_0,d[C_0])$ consisting of  stable maps
 $C\to C_0$ of degree $d$. This component is isomorphic to $\M(\P^1,d)$ and has
dimension $2d-2$. The virtual dimension
 of this component is zero. The obstruction sheaf $\F_d$ is a vector bundle of
rank $2d-2$ with the fiber at each point $f:C\to C_0$
 equal to
...
1-dimensional subschemes in $\P^1\times \P^1$
 (they associate with a map $f:\P^1\to \P^1$ its graph).
...
% \endhead
\section{Fixed points formulas}

% \subheading{3.1. The Bott fixed points formula}
\subsection{The Bott fixed points formula}
...
$\l:\T\to \C^{\times},\,\,\l\in\T^{\vee}\simeq\Z\oplus\dots\oplus\Z$. Also, the
normal bundle
...
degree $2$  obeying the relations
 $$\sum_{k\ge 0}c_k(\Cal E)=\prod_{i}(1+e_i)\,\,.$$ Analogously, we add
...
\subsection{Fixed points on moduli spaces of stable maps}
...
Let a stable map
 $f:C\to \P^n$ represent a point of $\M_{g,k}(\P^n,d)^{\T}$. First of all, the
...
\subsection{Contributions of connected components}
...
\begin{notation} For a flag $F=(v,\alpha)$ of $\Gamma$ we denote by $w_F$
the expression $(\lambda_{f_v}-\lambda_{f_u})/d_{\alpha}$ where $u\in
Vert(\Gamma),u\ne v$  is the second vertex of the edge $\alpha$.\end{notation}
...
\subsubsection{Intersection theory on $\M_{0,k}$}
...
\begin{lem} $\langle\tau_{d_1}\dots\tau_{d_k}\rangle_0=\frac
{(k-3)!}{d_1!\dots d_k!}\,\,.$
\end{lem}
...
\begin{cor} $I(w_1,\dots,w_k)=\dsize\prod_{i=1}^k w_i^{-1}
...
\subsubsection{The contribution of $\Cal N^{abs}_{\M^{\Gamma}}$}
...
\subsubsection{The contribution of $[H^0(C,f^*(\Cal T_{\P^n}))]$}
...
$\M^{\Gamma}$:
$$0\to H^0(C,f^*(\Cal T_{\P^n}))\to\dsize\bigoplus_{\alpha\in
Edge(\Gamma)}H^0(C^{\alpha},f^*(\Cal T_{\P^n}))\to
$$
$$\to\bigoplus_{v\in Vert (\Gamma)}
\left(T_{p_{f_v}}\P^n\otimes\C^{val(v)-1}\right)\to0$$
...
\subsubsection{The contributions of the vector bundles in the examples}
...
right):
$$H^0(C,f^*(\Cal O (5)))\to\dsize\bigoplus_{\alpha\in Edge(\Gamma)}
H^0(C^{\alpha},f^*(\Cal O (5)))\to\bigoplus_{v\in Vert (\Gamma)}
...
\left(\Cal O(-1)_{p_{f_v}}\otimes \C^{val(v)-1}\right)\to H^1(C,f^*(\Cal O
(-1)))\to\dsize\bigoplus_{\alpha\in Edge(\Gamma)}
H^1(C^{\alpha},f^*(\Cal O (-1)))
\,\,\,.$$

% \subheading{3.4. The final sum}
\subsection{The final sum}
...
\subsection{Feynman rules and summation over trees}
...
$$w(\Gamma):=\sum_{\substack{\text{maps }\\f:Flags(\Gamma)\to A}}
\left(
\prod_{\substack{
\alpha\in Edge(\Gamma)\\
...
g^{f(F_1)f(F_2)}
\prod_{\substack{
v\in Vert(\Gamma)\\F_1,\dots,F_k\text{ - flags}\\\text{ containing }v
}}
...
$$Z^{tree}:=\sum_{\substack{
\Gamma:\text{ equivalence classes of}
\\ \text{finite nonempty trees }\Gamma
}} \frac{1}{\# Aut(\Gamma)}\,w(\Gamma)\,\,.$$
...
\begin{formula} We have $Z^{tree}=Crit\,S(\phi_*)$, where the r.h.s. denotes
...
\subsubsection{The action functionals in the three examples}
...
(\tilde\phi_{\tilde j_1}\dots\tilde\phi_{\tilde j_l})\right)\,\,.$$
 As before, we multiply the matrix coefficients of $g$ by extra variables:
...
\subsection{The potential part of the action functional}
...
\begin{thm} We have $S(v_*,\phi_*)=Crit\,B(\xi)$, where
...
\subsubsection{The effect of marked points}
...
\subsection{The structure of the resulting functional}
...
%$A\times A$-matrix with corresponding elements on the diagonal.
...
changes of notation we get the following formula:
...
of trees disappear. The result of the computation   is equal to $Li_3(e^t)$.
...
\subsection{Flag spaces and toric varieties}
...
\textit{Topological field theory and rational curves},
\textit{Commun. Math. Phys.} \textbf{151} (1993), 245--262.
...
\textit{Riemann-Roch for singular varieties},
{Publ. Math. I. H. E. S.} \textbf{45} (1975), 101--145.
...
\bibitem[B]{B}
R. Bott,
\textit{A residue formula for holomorphic vector fields},
{Jour. Diff. Geom.} \textbf{1} (1967), 311--330.
...
{Publ. Math. I. H. E. S.} \textbf{36} (1969), 75--110.
...
\bibitem[GK]{GK}
A. Givental, B. Kim,
\textit{Quantum cohomology of flag manifolds and Toda lattices},
preprint (1993).
...
\textit{Statistical field theory},
...
\bibitem[K]{K}
M. Kontsevich,
\textit{Intersection theory on the moduli space of curves and the matrix Airy function},
{Commun. Math. Phys.} \textbf{147} (1992), 1--23.
...
\textit{Gromov-Witten classes, quantum cohomology and enumerative geometry},
MPI preprint and hep-th/9402147 (1994).
...
\bibitem[P]{P}
P. Pansu,
\textit{Chapter VIII, Compactness},
Holomorphic curves in symplectic geometry,
eds. M. Audin, J. Lafontaine, Progress in Mathematics, vol. 117,
{Birkh\"auser} (1994).
...
\bibitem[R]{R}
Z. Ran,
\textit{Enumerative geometry of singular plane curves},
{Invent. Math.} \textbf{97} (1989), 447--465.
...
\bibitem[RT]{RT}
Y. Ruan, G. Tian,
\textit{Mathematical theory of quantum cohomology},
preprint (1993).
...
{Surveys in Diff. Geom.} \textbf{1} (1991), 243--310.
...
\bibitem[Y]{Y}
S. T. Yau, ed.,
\textit{Essays on Mirror Manifolds},
International  Press Co., Hong Kong, 1992.
