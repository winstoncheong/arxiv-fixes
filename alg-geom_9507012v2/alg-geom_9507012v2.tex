...
\usepackage{geometry}
\usepackage[colorlinks,allcolors=blue,unicode,psdextra,pagebackref]{hyperref}

% Filling in commands that were probably defined in AMS-LaTeX
...
\end{equation*}
where $c_2\cap c'_2\in\mathbb Z$ is the natural pairing between
...
$x_i$ in $X$ with coefficients $n_i\in\mathbb Z_{> 0}$ with $\sum n_i = n$.
...
\begin{Theorem}
  ...
  vector $v_0$ is the generator of $\Hlf_0(\HilbX{0})\cong\mathbb Q$.
\end{Theorem}
...
\begin{gather*}
  0 \to \shfO(-1) \xrightarrow{z_0} \shfO \to \shfO_l \to 0, \\
  0 \to \shfO(-3) \xrightarrow{z_0} \shfO(-2) \to \shfO_l(-2) \to 0.
\end{gather*}
...
sense of \cite{HKLR},
where the vector space $\bM$ has a structure of a $\mathbb H$-module by
\begin{equation*}
  J (B_1, B_2, i, j) \defeq
...
\bibitem[Na3]{Na-resol} \bysame,
 {\it Resolutions of moduli spaces of ideal instantons on $\mathbb R^4$},
