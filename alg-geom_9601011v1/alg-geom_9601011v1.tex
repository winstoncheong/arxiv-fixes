...
and arbitrary genus are constructed using the techniques from
[K.~Behrend, B.~Fantechi. {\em The Intrinsic Normal Cone.\/}]}
\end{abstract}
\tableofcontents
...
\section{Preliminaries on Prestable Curves}
...
\end{prop}
\begin{proof} For the definition of $\dim(\tau)$ and $\chi(\tau)$ see \cite{BM},
...
\[\begin{tikzcd}
  \ol{M}(\tau') \dar \drar["\ol{M}(\phi)"] \\
  \MM(\tau) \rar["s"] & \ol{M}(\tau^s).
\end{tikzcd}\]
...
\section{The Virtual Fundamental Classes}
...
\end{align*} Essential is the following result.
...
$t\in T$ we have
\begin{align*}
  H^0(C_t,F) &= H^0(C_t,\pi\lst(E\otimes L^{\otimes N})_t\otimes L_t^{\otimes -N}) \\
             &= H^0(C_t,L_t^{\otimes -N})\otimes\pi\lst(E\otimes L^{\otimes N})_t  \\
             &= 0
\end{align*}
and hence $H^0(C_t,H)=0$,
...
\end{them}
\begin{proof} If $V$ is convex and $\tau$ a forest, then $R^1\pi\lst(f\upst
...
\section{Checking the Axioms} \label{check}

\subsection{{\sc Axiom I.} Mapping to a point}
...
\begin{align*}
R^1\pi\lst f\upst T_V&=T_V\boxtimes R^1\tilde{\pi}\lst\O_{\cC(\tau)}\\
&=\tT^{(1)}
\end{align*}
...
\subsection{{\sc Axiom II.} Products}
...
\subsection{{\sc Axiom III.} Cutting Edges}
...
% V\times V}\]
\[\begin{tikzcd}
\ol{M}(V,\sigma,\beta) \rar["\ol{M}(\Phi)"] \dar["g"']
& \ol{M}(V,\tau,\beta) \dar \\ 
\MM\times V \rar["\Delta"] & \MM\times V\times V
\end{tikzcd}\]
...
\[\begin{tikzcd}
\cC' \rar["p"] \drar["\pi"'] & \cC \dar["\pi"] \\
& \ol{M}(V,\sigma,\beta).
\end{tikzcd}\]
We shall also use the diagram
% \[\comtri{\cC'}{p}{\cC}{f'}{f}{V,}\]
\[\begin{tikzcd}
\cC' \rar["p"] \drar["f'"'] & \cC \dar["f"] \\
& V,
\end{tikzcd}\]
...
\[
\begin{tikzcd}
\ol{M}(\Phi)\upst E\com(V,\tau,\beta) \rar \dar &
E\com(V,\sigma,\beta) \rar \dar &
g\upst L_{\Delta} \rar["\dual{R\pi\lst(-u)}"] \dar &
\ol{M}(\Phi)\upst E\com(V,\tau,\beta)[1] \dar \\
\ol{M}(\Phi)\upst L\com_{\ol{M}(V,\tau,\beta)/\MM} \rar &
L\com_{\ol{M}(V,\sigma,\beta)/\MM} \rar &
L\com_{\ol{M}(\Phi)} \rar &
\ol{M}(\Phi)\upst L\com_{\ol{M}(V,\tau,\beta)/\MM}[1],
\end{tikzcd}
\]
...
\subsection{{\sc Axiom IV.} Forgetting Tails}
...
\[
\begin{tikzcd}
  \ol{M}(V,\sigma,\beta) \rar["\ol{M}(\Phi)"] \dar["d"'] &
  \ol{M}(V,\tau,\beta) \dar \\
  \cC \rar &
  \MM(\tau).
\end{tikzcd}\]
...
\[\begin{tikzcd}
  \ol{M}(V,\sigma,\beta) \rar["d"] \drar & \cC \dar \\
  & \MM(\sigma)
\end{tikzcd}\]
...
\[
\begin{tikzcd}
  \cC(V,\sigma,\beta) \rar["p"] \drar["\pi"'] & \ol{M}(\Phi)\upst \cC(V,\tau,\beta) \dar["\pi'"]\\
  & \ol{M}(V,\sigma,\beta)
\end{tikzcd}
\]
and
...
\[
\begin{tikzcd}
  \cC(V,\sigma,\beta) \rar["p"] \drar["f"'] & \ol{M}(\Phi)\upst \cC(V,\tau,\beta) \dar["f'"]\\
  & V .
\end{tikzcd}
\]
...
\subsection{{\sc Axiom V.} Isogenies}
...
\[\begin{tikzcd}
  \MM(\tau') \dar["s"'] \rar["\MM(\Phi')"]
  & \MM(\sigma') \dar["s"] \\
  \ol{M}(\tau) \rar["\ol{M}(\Phi)"]
  & \ol{M}(\sigma).
\end{tikzcd}\]
...
%\end{array}\]
\[\begin{tikzcd}
  \cC \dar \rar["\pi'"] & \MM(\sigma') \arrow[dd,"s"] \\
  \MM(\tau') \dar["s"'] \\
  \ol{M}(\tau) \rar["\ol{M}(\Phi)"] & \ol{M}(\sigma)
\end{tikzcd}\]
...
\subsection{Curves and Principal Bundles}
...
%\[\comdia{\MM(G/P,\tau,\beta)/G^{V_{\tilde{\tau}}}}{\kappa}{\HH^1_{\beta}(\tau,P)}{\eta}{}{}{\MM(\tau)/G^{V_{\tilde{\tau}}}}{\iota}{\HH^1(\tau,G).}\]
\[\begin{tikzcd}
  \MM(G/P,\tau,\beta)/G^{V_{\tilde{\tau}}} \dar["\eta"'] \rar["\kappa"] & \HH^1_{\beta}(\tau,P) \dar \\
  \MM(\tau)/G^{V_{\tilde{\tau}}} \rar["\iota"] & \HH^1(\tau,G).
\end{tikzcd}\]
...
L\com_{\HH^1_{\beta}(\tau,P)/\HH^1(\tau,G)}. \qedhere
\]
\end{proof}
...
\newblock {G}romov-{W}itten classes, quantum cohomology, and enumerative
  geometry.
\newblock {\em Communications in Mathematical Physics}, 164:525--562, 1994.
