...
First of all, it is necessary to slightly twist the multiplication in $\H_{\A} \otimes \H_{\A}$~: if $x, y, z, w$ are homogeneous elements in $\H_{\A}$ of respective weight $wt(x), wt(y), wt(z),$ $wt(w) \in K(\A)$, we define a new multiplication by
...
${D}_{(K_1,K_2,L_1,L_2)}$. For instance, when $\mathcal{A}$ is the category of modules over some ring, we have $L_1 \simeq L\cap N, \;L_2 \simeq L/(L\cap N), \;K_1\simeq N/(L \cap N)$ and $K_2 \simeq J/(L \oplus N)$. The map $\Phi$ defined above descends to the desired map $\widetilde{\Phi}$ between equivalence classes of crosses and squares.
...
A sufficient condition for these two quantities to be equal, and hence for $f^*$ to be a morphism of algebras, is that~: $F$ preserves Euler forms (i.e. $\langle M, N \rangle_m = \langle F(M), F(N) \rangle_m$ for any $M,N \in \mathcal{X}_{\A}$) and that for any $R \in \mathcal{X}_{\A}$, the functor $F$ sets up a bijection between the subobjects of $R$ and $F(R)$.
...
Though we have not explained the definition of the Hall algebra of an exact category, let us give a last (essentially obvious) functoriality property of Hall algebras.
...
$P^{(1^t)}_{(1^{t-r}),(1^r)}$ is equal to the number of points over $\mathbb{F}_q$ of the Grassmannian $Gr(r,t)$ of $r$-planes in $t$-space. This number is equal to a $q$-binomial coefficient
...
\end{split}
\end{equation}
To see why this is true, let us pick arbitrary subspaces $S_{n-1}, \ldots, S_1, S_0$ in the above product of Grassmannians and look at the fiber of $\pi$ at that point. For $R$ to belong to it, we need to have in particular $R \cap K_{n-1}=S_{n-1}$ and $(R \cap K_{n-2})/(R \cap K_{n-1})=S_{n-2} \in K_{n-2}/K_{n-1}$. Fixing a basis of $S_{n-1}$ and $S_{n-2}$ we see that $R \cap K_{n-2}$ is determined up to a choice of a linear map from $S_{n-2}$ to $K_{n-1}/(R \cap K_{n-1})$. But then once $(R \cap K_{n-2})$ and $S_{n-3}$ are fixed, $(R \cap K_{n-3})$ is determined up to a linear map from $S_{n-3}$ to $K_{n-2} / (R \cap K_{n-2})$, and so on. Continuing in this manner, we arrived at the desired result.

Finally, since the number of points over $\mathbb{F}_q$ of the Grassmannian $Gr(r,t)$ is $\begin{bmatrix} t \\ r \end{bmatrix}_+$, we obtain \textit{in fine}
...
Despite these seemingly complicated formulas, the (abstract) structure of $\H_{cl}$ is startlingly simple~:
...
\subsection{Other occurrences of Hall algebras.}
...
Hence, geometrically, the set of indecomposables resembles a projective line with three points being ``doubled''~:
...
\begin{theo}[Ringel, Green]\label{T:RingelHall} The assignment $E_i \mapsto [S_i],\; K_i \mapsto \mathbf{k}_{S_i}$ for $i \in I$ extends to an embedding of Hopf algebras
...
Using the above Proposition, we see that the set of objects of $Rep^{nil}_k\vec{Q}$ is in canonical bijection with the set $\Pi^n$ of $n$-tuples of partitions $(\underline{\lambda}_1, \ldots, \underline{\lambda}_n)$, via the assignment
...
To finish, let us indicate a second decomposition of $\H_{\vec{Q}}$, as a noncommutative product this time. Let $\mathcal{C}$ be the full subcategory of $Rep_k\vec{Q}$ consisting of those representations $(V,\underline{x})$ for which $x_i: V_i \to V_{i-1}$ is an isomorphism for all $i \neq 1$. In other words, objects of $\mathcal{C}$ are all direct sums of indecomposables of the form $I_{[0;nl]}$ for some $l \geq 1$. It is clear that $\mathcal{C}$ is an abelian subcategory closed under extensions, and which is equivalent to $Rep_k\vec{Q}_0$ where $\vec{Q}_0$ is the Jordan quiver. Thus by Corollary~\ref{C:cathall} there is an embedding of algebras
\begin{align*}
...
\begin{theo}[Kapranov, \cite{Kap1}]\label{T:Kap} The assignment $E_l \mapsto [\O(l)]$ for $l \in \Z$, $H_r \mapsto T_r\mathbf{k}_{\delta}^{-|r|/2}$ for $r \geq 1$, $K \mapsto \mathbf{k}_{\O}$ and $C^{1/2} \mapsto \mathbf{k}_{\delta}^{1/2}$ extends to an embedding of algebras $$\Psi:\U_{\nu}(\mathcal{L}\bo_+) \to \widetilde{\H}'_{\mathbb{P}^1}.$$
\end{theo}
\noindent
...
With a little algebraic manipulation, we may rewrite the formula (\ref{E:product1rdo}) as
...
The above definition of $\theta_l$ is not very pleasant or useful.
...
where $S(\p,\llambda)\mathrm{-}Fin$ is the Serre subcategory of $S(\p,\llambda)\mathrm{-}Modgr$ consisting of finite-dimensional graded $S(\p,\llambda)$-modules. In plain terms, this means that, just as for $Coh(\mathbb{P}^1)$, to any finitely-generated graded $S(\p,\llambda)$-module $M=\bigoplus_{\vec{x} \in L(\p)} M_{\vec{x}}$ is associated a sheaf $M\;\widetilde{}$, and two such modules $M,N$ correspond to the same sheaf if $M_{\geq n}:=\bigoplus_{\vec{x} \geq n\vec{c}} M_{\vec{x}} \simeq N_{\geq n}:=\bigoplus_{\vec{x} \geq n\vec{c}} N_{\vec{x}}$. Moreover, the spaces of homomorphisms are computed as follows~:
...
\addtocounter{theo}{1}
\paragraph{\textbf{Remark \thetheo.}} As mentioned at the beginning of this Section, there are other ways of defining the weighted projective lines~:\\
...
\begin{theo}[S., \cite{SDuke}]\label{T:SDuke} The assignment
...
\end{split}
\end{equation*}
where $\O_{\lambda_i}^{(m_i,l_i)}$ is the unique indecomposable  torsion sheaf supported at $\lambda_i$ of length $m_ip_i+l_i$ and socle $S_{i;0}$; and where the first sum ranges over all tuples of distinct ordinary points $x_j$ and multiplicities $n_j, m_i$ for which $\sum_j n_jdeg(x_j)+\sum_i m_i=l$.
\end{lem}
\noindent
...
The purpose of this section is to introduce an important technical notion for studying the categories of coherent sheaves on smooth projective curves. Throughout this section, $X$ will denote either a smooth projective curve or an (arbitrary) weighted projective line. A good reference here is \cite{Seshadri}.
...
\end{lem}
\noindent
\textit{Proof.} Since any torsion sheaf is semistable of slope $\infty$, a semistable sheaf of slope $\eta <\infty$ is automatically a vector bundle. Hence if $\mathcal{F}=\mathcal{V} \oplus \mathcal{T}$ is a decomposition of $\mathcal{F}$ as a direct sum of a vector bundle and a torsion sheaf then we put $\mathcal{F}_1=\mathcal{T}$ and we are reduced to proving the Lemma for $\mathcal{V}$. Thus we might as well assume from the beginning that $\mathcal{F}$ is a vector bundle.
...
\begin{cor}\label{C:crucial} Assume that $X$ is either a smooth projective curve of genus at most one, or that $X$ is a weighted projective line of parabolic or elliptic type. Then
\begin{enumerate}
...
\end{lem}
\noindent
\textit{Proof.} By Corollary~\ref{C:crucial} and the remark following it, it suffices to to choose any total ordering refining the partial ordering according to slope.\qed
...
where $\mathcal{C}_{\eta,x} \simeq Rep^{nil}_{k_x}\vec{Q}_0$ and $\mathcal{C}_{\eta,\lambda_i} \simeq Rep^{nil}_kA_{p_i-1}^{(1)}$ are respectively equivalent to the categories of nilpotent representations of the Jordan quiver ot the cyclic quiver of length $p_i$. As a corollary of the splitting of the Harder-Narasimhan filtration (see Corollary~\ref{C:crucial}) there is an isomorphism of vector spaces
...
Let as usual $k=\mathbb{F}_q$ be a finite field and set $\nu=q^{\frac{1}{2}}$. In defining the extended Hall algebra we run into a new phenomenon: the Grothendieck group $K(Coh(X))$ is infinite-dimensional. This is not a serious issue~: by the Riemann-Roch theorem, the Euler form is
...
\begin{theo}[Burban-S.]\label{T:BS}  The assignment $T_{(p,q)} \mapsto {t}_{p,q}$ for $(p,q) \in (\Z^2)^+$ induces an isomorphism between $\CC_X$ and the algebra $\U^+_X$ generated by the elements $\{{t}_{(p,q)}\;|\; (p,q) \in (\Z^2)^+\}$ subject to the following set of relations
\begin{enumerate}
...
\vspace{.15in}
 ...
 Although the categories of representations of quivers (discussed in Lecture~3) and the categories of coherent sheaves on smooth projective curves (which form the content of Lecture~4) are of very different origin there are, as the reader has beyond doubt already noticed, some striking similarities.
...
This explains the occurrence of affine Lie algebras $\widehat{\g}$ in both contexts, and matches the picture of regular indecomposables for tame quivers and the picture for torsion sheaves in weighted projective lines. It also provides a conceptual explanation for the existence of a total order on the set of indecomposables of a quiver of finite type satisfying the conditions stated in Lemma~\ref{L:orderfinite} (it suffices to choose any order refining the order given by the slope function).
...
\begin{conj}\label{Conj:2} Let $F: D^b(\mathcal{C}_1) \to D^b(\mathcal{C}_2)$ be an exact, fully faithful and friendly functor. For any object $M$ of $\mathcal{C}_1$, define $n \in \Z$ and objects $N, N'$ of $\mathcal{C}_2$ such that $F(M) \simeq N[-n] \oplus N'[-1-n]$. Then, setting $\epsilon(n)=-(1)^n$, the assignment
...
the structure sheaf of a closed point $\O_{x_0}$ of degree one. Consider the functor
$$
...
From the definitions, it is clear that the assignment $[M]^{(i)} \mapsto [M]^{(i+1)}, \mathbf{k}_{\a} \mapsto \mathbf{k}_{\a}^{-1}$ gives rise to an automorphism $\Sigma~: L(\A) \stackrel{\sim}{\to} L(\A)$.
...
These new relations are called the \textit{Serre relations.} The only question that now remains is~: which are the possible values for the Cartan matrix $A$ ? This
...
It is clear that $\widehat{\bo}$ is not conjugate to $\widehat{\bo}_{\pm}$ under the affine Weyl group $\widehat{W}$ since $\widehat{\bo}_{\pm}$ is finitely generated whereas (by weight considerations) $\widehat{\bo}$ is not.
...
\paragraph{\textbf{Proposition~A.19.}}(Garland, \cite{Garland}). \textit{Let $\g$ be a simple Lie algebra. The affine Lie algebra $\widehat{\g}$ is isomorphic to the Lie algebra generated by $\{e_{i,k},f_{i,k},h_{i,k}\}$ for $i \in I$ and $k \in \Z$, and $c$ subject to the following relations~:
...
The isomorphism with $\widehat{\g}$ is induced by the assignment $e_{i,k} \mapsto e_i \otimes t^k, f_{i,k} \mapsto f_i \otimes t^k, h_{i,k} \mapsto h_i\otimes t^k, c \mapsto c$.
...
The assignment $e_{i,k} \mapsto e_i \otimes t^k, f_{i,k} \mapsto f_i \otimes t^k, h_{i,k} \mapsto h_i\otimes t^k, c \mapsto c$ gives a surjective morphism of Lie algebras
...
$$\sum_{k \geq 0} \psi_{i,k}u^k=K_i \exp\bigg( (v-v^{-1})
\sum_{k=1}^\infty H_{i,k}u^k\bigg),$$
$$\sum_{k \geq 0} \varphi_{i,k}u^k=K_i^{-1} \exp\bigg( -(v-v^{-1})
\sum_{k=1}^\infty H_{i,-k}u^{-k}\bigg).$$}
...
H. Lenzing, \emph{Hereditary categories}, lecture notes, available at \url{people.math.jussieu.fr/~keller/ictp2006/schedule.html}
...
\bibitem[SV1]{SV}
O. Schiffmann, E. Vasserot, \emph{The elliptic Hall algebra, Cherednik Hecke algebras and Macdonald polynomials}, preprint  arXiv:0802.4001 (2008).
...
\bibitem[XY]{XiaoBGP}
J. Xiao, S. Yang, \emph{BGP-reflection functors and Lusztig's symmetries: A Ringel-Hall approach to quantum groups}, J. Algebra \textbf{241}, 204--246, (2001).
